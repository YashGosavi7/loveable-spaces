    ...
    >
      <Link to={`/portfolio/${id}`} className="group block relative">
        ...
        <div className="absolute inset-0 bg-darkGray/70 opacity-0 group-hover:opacity-100 
          ...
          <p className="text-white/90 mb-2 md:mb-3 text-sm md:text-base">
            {category} | {location}
            {designer && ` | Designed by ${designer}`}
          </p>
          
          {/* Project Title - Displayed prominently on hover */}
          <p className="text-roseGold mb-3 font-playfair text-xl md:text-2xl">{title}</p>
          
          {tagline && (
            <p className="text-white/80 mb-4 italic text-sm">{tagline}</p>
          )}
